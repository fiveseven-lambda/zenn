...
# `..`
`for` 式で `in` の後に書けるものとして，今までに「配列への参照」「ベクタへの参照」の 2 つが登場しました．ここではもう一つ紹介します．
...
```
先ほどと同様に，負であるようなものを読み込んだ時点で終了しますが，そのときに `x` の値を返しています．これが `value` に代入され，最後に `println!` マクロで出力されてます．
...
1. $x = 0$ より $x > 0$ が成り立たないので，ループが終了します．

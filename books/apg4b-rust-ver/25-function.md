...
:::message
ブロックが値を返すときと同じく， `digits()` の返したベクタが `vec` に代入される際にムーブが起こっています．
:::
## `()` を返す関数
たとえば，
```rust
...
```
# 環境
「今どんな変数が存在し，各変数がどんな型/値をもっているか」という情報を，**環境**といいます．たとえば，
```rust
fn main() {
    let a = 10;
    let b = 20;
...
```
というコードの中で， `println!` の行における環境は「変数 `a` と `b` が存在し，それぞれの値は `i32` 型の 10 と `i32` 型の 20」になります．

環境は，関数ごとに独立です．次のコードを見てください．
```rust
fn main() {
    let a = 10;
    let b = 20;
...
```
`main` 関数と `fnc` 関数は別の関数なので，これらは独立の環境をもちます．どういうことかというと， `fnc` 関数の `println!` の行における環境は，「変数 `c` が存在し，その値は `i32` 型の 30」であって，ここに `main` 関数の環境は含まれないということです．

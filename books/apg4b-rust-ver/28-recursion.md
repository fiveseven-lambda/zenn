...
関係式 $n! = (n - 1)! \times n$ が成り立つのは， $n \geq 1$ のときでした．よって $n = 0$ のときは `fact(n - 1) * n` とすることができないので，代わりに $0! = 1$ を返すようにします．こうすると， `fact(3)` から始まった呼び出しのループは， `fact(1)` が `fact(0)` を呼び出したところで終了することになります．

今回の `fact(0)` のように，再帰を終了させるために作られる場合分けをベースケースといいます．再帰関数は，呼び出しを繰り返すことでいつか必ずベースケースに到達しなければなりません．そうでなければ呼び出しが無限に繰り返され，スタックオーバーフローと呼ばれるエラーになります．
...
全体では次のようになります． `x` がスライスのとき， `x.is_empty()` は `x` が空スライスなら `true` に，そうでないなら `false` になります．
```rust
fn main() {
...
```
# 例 3: クイックソート
`x` が可変なスライスのとき， `x.sort()` を呼び出すと `x` の要素が昇順にソートされる（小さい順に並べ替えられる）のでした．これと同じことをする関数を作ってみます．ソートのやり方は数多くありますが，今回は再帰関数を使ってクイックソートと呼ばれるものを書いてみます．
```rust
...
## 分割
ここからスライスに対して `swap` 関数を使った操作を繰り返し，次のような状態を目標とします．

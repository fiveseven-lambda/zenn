...
さて，最初の問題に戻って， 2 つのアルゴリズムの計算量 $f_2(N), f_4(N)$ をランダウの $O$ を用いて評価してみます．このとき， $f(N) = O(g(N))$ の $g(N)$ が単純な式になるようにします．解法 1 のアルゴリズムについては， $f_2(N) \leq 9(N + 1)^3 = 9(N^3 + 3N^2 + 3N + 1)$ なので $f_2(N) = O(N^3)$ です．解法 2 のアルゴリズムについては， $f_4(N) \leq 9(N + 1)^2 = 9(N^2 + 2N + 1)$ なので $f_4(N) = O(N^2)$ です．

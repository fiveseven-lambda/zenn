...
一方，文字列リテラル `"2 + 3 = {}"` はあくまで「`2`，` `，`+`，` `，`3`，…」という文字の並びを意味するため，そこに含まれる `2 + 3` が計算されて `5` になることはありません．文字列について詳しくは[第 23 章](https://zenn.dev/toga/books/rust-atcoder/viewer/23-string)で説明します．

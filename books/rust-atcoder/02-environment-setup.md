...
:::message
大抵のシェルには補完機能というものがあり，`Documents` という名前を全て入力しなくても，`Doc` まで入力して Tab キーを押すと（他に `Doc` で始まるディレクトリが無い限り）`Document` まで自動で補完されます．長い名前もミスせず入力できるため便利です．
:::
...
さて，この状態でビルドを行ってみましょう．次のコマンドを実行してください．
```
$ cargo build
```
...
`cargo build` の代わりに `cargo run` を打つと，ビルドと実行がいっぺんに行われます．
```
$ cargo run
```

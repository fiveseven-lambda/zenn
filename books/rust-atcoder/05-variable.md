...
よって，変数を使うには，ビット列で表された値の他に，「そのビット列をどう解釈すべきか」という情報も必要になります．これを**型**といいます．
...
## 静的型付け
プログラム中に現れる全ての変数や式の型は，コンパイルの段階で決まります．コンパイルの結果できたプログラムを実行すると，単純な演算が組み合わされて複雑な計算が進んでいきますが，そこに現れる変数や式の型は，全て実行する前に決まっているのです．

このことを，**静的型付け**といいます．
## 異なる型の代入や計算
異なる型の間で代入を行おうとするプログラムを書くと，コンパイルエラーになります．たとえば，
```rust:コンパイルエラー
fn main() {
    let hoge: f64;
    hoge = 2;
...
実は， `hoge` の型を明示的に書かなくても，その後の `hoge = 10;` という文から hoge の型が推測されるようになっています．これを**型推論**といいます．今回は整数値の 10 を代入しているので， `hoge` は `i32` 型であればよいと判断されます．
...
let hoge = 10;
```
と書くと， `hoge` という変数が宣言されるとともに， `hoge` に `10` が代入されます．英語の "Let $x = 10$."（$x = 10$ とする）という表現と同じ形になります．

...
`fnc1() && fnc2()` は， `fnc1()` と `fnc2()` が両方`true` だったときにのみ `true` を返します．しかし，今回 `fnc1()` は `false` だったので， `fnc2()` の返り値にかかわらず `fnc1() && fnc2()` は `false` になるということが分かります．このとき， `&&` 演算子は，`fnc2()` の呼び出しを省略して直ちに `false` を返します．これを**短絡評価**といいます．

短絡評価の使い方を説明するために，まず以下のコードを考えます．
```rust
use proconio::input;
...
```
`x` として受け取った整数が `3` のとき，`x != 0` も `12 % x == 0` も `true` なので，これらを `&` でつないだ式も `true` となり，`3 は 12 の約数です` と出力されます．一方，`x` として受け取った整数が `5` のとき，`x != 0` は `true` ですが `12 % x == 0` は `false` なので，これらを `&` でつないだ式は `false` となり，何も出力されません．

ここで，もし `x` として `0` を入力されたらどうなるでしょうか．途中で `12 % x` を計算しようとした時点で，ゼロ除算による実行時エラーが起こってしまいます．
...
```
`x` として `0` を入力された場合，`x != 0` が `false` なので，`&&` 演算子は残りの `12 % x == 0` を計算することなく `false` を返します．よって，ゼロ除算による実行時エラーは起こらないのです．短絡評価はこうして利用します．

...
:::message
このコードにおいて，`f64` 型の値 `x` が `1.5` に等しいことをアサートするために
```rust
assert_eq!(x, 1.5);
```
ではなく
...
```
と書いています．`f32` や `f64` の計算は誤差を伴うため，たとえば，次のコードを実行すると
```rust
fn main() {
    let x = 15. / 11.;
    assert_eq!(x * 11., 15.);
...
thread 'main' panicked at 'assertion failed: `(left == right)`
 ...
 right: `15.0`', src/main.rs:3:5
```
コンピュータの内部で $\frac{15}{11}$ が正確に表現できず， 11 倍したときに元の値と少しズレてしまうからです．

そこで，`x` と 1.5 を `==` で比較する代わりに，`x` と 1.5 の差が $10^{-6}$ 以下であれば等しいと見なしています．
:::

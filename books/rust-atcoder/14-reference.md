...
:::message
今まで， `x.sin()` や `x.rem_euclid(y)` のように，変数 `x` の後にピリオド `.` を置いてそのあとに `sin()` や `rem_euclid(y)` などと続けるような書き方がいくつか登場しました．これは後の章で説明する関数呼び出しの形の一つで，このときも `x` は自動的に参照外しされることがあります．たとえば， `x` の型が `&f64` のとき， `x.sin()` は `(*x).sin()` という意味になります．これを**暗黙の参照外し**といいます．
:::
...
ここまでに述べたルールによって，参照外しをするとき，そのアドレスには必ず*有効な値*が入っていることが保証されます．参照されるアドレスに，未初期化の値やスコープの終了した変数の値が入っているようなことは，借用とライフタイムのルール上起こりえないのです．逆に言うとこれらのルールは，参照外しが常に問題なく行えるために設けられています．

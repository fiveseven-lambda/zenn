...
また，途中で `40_i64` を追加しているので `vector` の型は `Vec<i64>` であると推論され，他の整数リテラル `10` `20` `30` `40` `50` も全て `i64` 型になります．
...
# for 式
`for` 式で `in` の後に書けるものとして，今までに「配列への参照」と「`a..b` のような範囲」がありました．ベクタの参照も，ここに書くことができます．
```rust
fn main() {
